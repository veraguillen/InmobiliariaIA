...
def main():
    ...
    #Solicitar al usuario que ingrese el ZPID
    zpid = input("Ingrese el ZPID de la propiedad: ").strip()
    if not zpid.isdigit():
        print_color("Error: El ZPID debe ser un número válido.", color="red", bold=True)
        return
    ...
    if property_details and property_details.get('status'):
        ...
        print_color("No se pudieron obtener los detalles de la propiedad.", color="red", bold=True)
